...
bool init_websocket_connection() {
    ...
    while (true) {
        ...
        int socket_fd = accept_connection(server_fd, &address, &addrlen);
        if (socket_fd < 0) {
        ...
        std::cout << "New client connected, FD: " << socket_fd << "\n";

        char buffer[1024] = {0};
        ssize_t bytes_received = receive_data(socket_fd, buffer, sizeof(buffer));
        if (bytes_received <= 0) {
        ...
        if (handle_websocket_upgrade(socket_fd, request)) {
            client_socket = socket_fd;
            std::cout << "Set client_socket = " << client_socket << "\n";
            return true;
        }
        ...
        std::string response =
            ...
            "\r\n" + html;
        if (send_data(socket_fd, response) < 0) {
